...
pub async fn emit<T: Serialize>(event: &str, payload: &T) -> crate::Result<()> {
    base::emit(event, serde_wasm_bindgen::to_value(payload)?).await?;
    ...
    T: DeserializeOwned + 'static,
{
    ...
    let unlisten = base::listen(event, &closure).await?;
    closure.forget();
    ...
    T: DeserializeOwned + 'static,
{
    ...
    let unlisten = base::once(event, &closure).await?;
    closure.forget();
...
mod base {
    ...
    #[wasm_bindgen(module = "/src/scripts/api/event.js")]
    extern "C" {
        #[wasm_bindgen(catch)]
