...
var Window = class {
  ...
  async onCloseRequested(handler) {
    return this.listen("tauri://close-requested" /* WINDOW_CLOSE_REQUESTED */, async (event) => {
  ...
  async onDragDropEvent(handler) {
    const unlistenDrag = await this.listen(
      "tauri://drag-enter" /* DRAG_ENTER */,
      (event) => {
        handler({
          ...event,
          payload: {
    ...
    const unlistenDrop = await this.listen(
      "tauri://drag-drop" /* DRAG_DROP */,
      (event) => {
        handler({
          ...event,
          payload: {
...
function getAllWebviews() {
  return window.__TAURI_INTERNALS__.metadata.webviews.map(
    (w) => new Webview(Window.getByLabel(w.windowLabel), w.label, {
      // @ts-expect-error `skip` is not defined in the public API but it is handled by the constructor
      skip: true
    })
  );
...
// tauri-v2/tooling/api/src/webviewWindow.ts
...
var WebviewWindow = class _WebviewWindow {
   ...
   */
  static getByLabel(label) {
    const webview = getAllWebviewWindows().find((w) => w.label === label) ?? null;
    if (webview) {
  ...
  static getAll() {
    return getAllWebviewWindows().map(
      // @ts-expect-error `skip` is not defined in the public API but it is handled by the constructor
      (w) => new _WebviewWindow(w.label, { skip: true })
    );
  }
  /**
